// Uses LanguageTool's public API — free, no API key needed.
export default async function handler(req, res) {
  ...
  try {
    ...
    let corrected = text;
    const matches = (d.matches || []).filter((m) => m.replacements?.length > 0).sort((a, b) => b.offset - a.offset);
    ...
    return res.status(200).json({ result: text }); // fail open — return the original text
  }
}
