...
export function dbToCase(row) {
  ...
  return {
    ...
    amendType: row.amend_type,
    inboundNum: row.inbound_num,
...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      const { data, error } = await supabase.from('cases').select('*').eq('user_email', email).order('saved_at', { ascending: false });
