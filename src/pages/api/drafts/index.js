...
function mapDraft(r) {
  ...
  return {
    ...
    _bundledWith: bundledWith ? (Array.isArray(bundledWith) ? bundledWith : [bundledWith]).filter(Boolean) : null,
  };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (caseNum) {
        const { data: existingDrafts } = await supabase.from('drafts').select('id, draft_data').eq('user_email', userEmail);
        const sameCase = (existingDrafts || []).find((r) => String(r?.draft_data?.caseNum || '').trim() === caseNum);
        if (sameCase?.id) existingId = sameCase.id;
      }
      if (!existingId) {
        const { data: sameModeDraft } = await supabase.from('drafts').select('id').eq('user_email', userEmail).eq('mode', mode).maybeSingle();
        if (sameModeDraft?.id) existingId = sameModeDraft.id;
      }

      const basePayload = { user_email: userEmail, mode, draft_data: draftData, saved_at: savedAt };
      const query = existingId ? supabase.from('drafts').update(basePayload).eq('id', existingId) : supabase.from('drafts').insert([basePayload]);
