...
function NavBadge({ children }) {
  return (
    <span className="inline-flex items-center justify-center min-w-[16px] h-[16px] px-1 rounded-full bg-ch-red text-white text-[9px] font-label font-bold leading-none">
      {children}
    </span>
  );
}

/** A single sidebar entry — icon, hover tooltip, active-state rail, optional count badge. */
function NavItem({ item, isActive, count }) {
  return (
    ...
    >
      {isActive && <span className="absolute right-0 top-1/2 -translate-y-1/2 h-[29px] w-[3px] rounded-l-[10px] bg-ch-main" />}
      <Icon name={item.icon} size={22} color={isActive ? '#40513B' : '#40513B99'} />
      {!!count && (
        <span className="absolute top-1.5 right-1.5">
          <NavBadge>{count}</NavBadge>
        </span>
      )}
      <span
        className={cls(
...
/** The 84px navigation rail — pixel-matched to the Figma "Sidebar v2" component. */
function Sidebar({ counts = {}, customLinks = [], onLogout }) {
  ...
  return (
    <aside className="flex flex-col items-center w-[84px] shrink-0 self-stretch bg-white border-2 border-white pt-[60px] pb-5 px-2.5 sticky top-0 z-10">
      <div className="flex flex-col items-center gap-5 w-16 flex-1">
        ...
        {NAV_GROUPS.map((group) => (
          <div key={group.label} className="flex flex-col gap-[5px] items-center w-full">
            <p className="text-[10px] font-label font-bold uppercase text-ch-main opacity-60">{group.label}</p>
            {group.items.map((item) => (
              <NavItem key={item.id} item={item} isActive={router.pathname === item.href} count={counts[item.id]} />
            ))}
          </div>
        ))}
      ...
      <button onClick={onLogout} className="w-full flex items-center justify-center gap-2.5 px-5 py-2.5 rounded-ch bg-ch-red border border-ch-red shadow-ch" aria-label="Log out">
 ...
 * area. Matches the Figma "Desktop" frame (Sidebar v2 + Body). Also acts as
 * the auth guard: renders nothing and redirects to /login until a session
 * is confirmed, so protected content never flashes for a logged-out
 * visitor — every page using AppLayout is automatically protected.
 */
export default function AppLayout({ children, counts, customLinks }) {
  ...
  return (
    <div className="flex min-h-screen bg-ch-background">
      ...
      <main key={router.pathname} className="flex-1 flex flex-col gap-2.5 items-start p-5 rounded-ch min-w-0 ch-animate-page">{children}</main>
    </div>
  );
}
